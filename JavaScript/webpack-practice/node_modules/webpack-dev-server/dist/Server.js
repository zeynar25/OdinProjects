...
const cjsRequire = (0, _nodeModule.createRequire)(require('url').pathToFileURL(__filename).toString());
...
function loadWebpackPeer() {
  webpackPeer ??= import("webpack").then(m => m.default).catch(() => undefined);
  return webpackPeer;
}
 ...
 */
const encodeOverlaySettings = setting => typeof setting === "function" ? encodeURIComponent(setting.toString()) : setting;
...
class Server {
  ...
  constructor(options, compiler) {
    ...
    (0, _schemaUtils.validate)(/** @type {Schema} */_options.default, options, {
      name: "Dev Server",
  ...
  static findIp(gatewayOrFamily, isInternal) {
    if (gatewayOrFamily === "v4" || gatewayOrFamily === "v6") {
      let host;
      const networks = Object.values(_nodeOs.default.networkInterfaces()).flatMap(networks => networks ?? []).filter(network => {
        ...
        if (typeof isInternal !== "undefined" && network.internal !== isInternal) {
          return false;
        }
        if (gatewayOrFamily === "v6") {
          const range = _ipaddr.default.parse(network.address).range();
          if (range !== "ipv4Mapped" && range !== "uniqueLocal" && range !== "loopback") {
            return false;
          }
        }
    ...
    const gatewayIp = _ipaddr.default.parse(gatewayOrFamily);

    // Look for the matching interface in all local interfaces.
    for (const addresses of Object.values(_nodeOs.default.networkInterfaces())) {
      for (const {
        cidr
      } of (/** @type {NetworkInterfaceInfo[]} */
      addresses)) {
        const net = _ipaddr.default.parseCIDR(/** @type {string} */cidr);
        if (net[0] && net[0].kind() === gatewayIp.kind() &&
  ...
  static async getFreePort(port, host) {
    ...
    const {
      default: getPort
    } = await Promise.resolve().then(() => require("./getPort.js"));
    const basePort = typeof process.env.WEBPACK_DEV_SERVER_BASE_PORT !== "undefined" ? Number.parseInt(process.env.WEBPACK_DEV_SERVER_BASE_PORT, 10) : 8080;

    // Try to find unused port and listen on it for 3 times,
    // if port is not specified in options.
    const defaultPortRetry = typeof process.env.WEBPACK_DEV_SERVER_PORT_RETRY !== "undefined" ? Number.parseInt(process.env.WEBPACK_DEV_SERVER_PORT_RETRY, 10) : 3;
  ...
  static findCacheDir() {
    ...
    for (;;) {
      try {
        if (_gracefulFs.default.statSync(_nodePath.default.join(dir, "package.json")).isFile()) break;
    ...
    if (!dir) {
      return _nodePath.default.resolve(cwd, `.cache/${pluginName}`);
    } else if (process.versions.pnp === "1") {
      return _nodePath.default.resolve(dir, `.pnp/.cache/${pluginName}`);
    } else if (process.versions.pnp === "3") {
      return _nodePath.default.resolve(dir, `.yarn/.cache/${pluginName}`);
    }
    return _nodePath.default.resolve(dir, `node_modules/.cache/${pluginName}`);
  }
  ...
  static isWebTarget(compiler) {
    ...
    return Boolean(platform.web || /** @type {{ universal?: boolean | null }} */platform?.universal || compiler.options.target !== false && platform.web === null && platform.node === null);
  }
  ...
  async addAdditionalEntries(compiler) {
    ...
    if (this.options.client && isWebTarget) {
      ...
      if (this.options.webSocketServer) {
        ...
        if (typeof client.overlay !== "undefined") {
          const overlayString = typeof client.overlay === "boolean" ? String(client.overlay) : JSON.stringify({
            ...client.overlay,
    ...
    const webpack = compiler.webpack || (/** @type {NonNullable<Awaited<ReturnType<typeof loadWebpackPeer>>>} */
  ...
  async normalizeOptions() {
    ...
    const getStaticItem = optionsForStatic => {
      const getDefaultStaticOptions = () => ({
        directory: _nodePath.default.join(process.cwd(), "public"),
        staticOptions: {},
        publicPath: ["/"],
        serveIndex: {
          icons: true
      ...
      if (typeof optionsForStatic === "undefined") {
        ...
        item = {
          ...
          publicPath: typeof optionsForStatic.publicPath !== "undefined" ? Array.isArray(optionsForStatic.publicPath) ? optionsForStatic.publicPath : [optionsForStatic.publicPath] : def.publicPath,
          serveIndex:
          ...
          typeof optionsForStatic.serveIndex !== "undefined" ? typeof optionsForStatic.serveIndex === "boolean" && optionsForStatic.serveIndex ? def.serveIndex : typeof optionsForStatic.serveIndex === "object" ? {
            ...def.serveIndex,
            ...optionsForStatic.serveIndex
          } : optionsForStatic.serveIndex : def.serveIndex,
          watch: typeof optionsForStatic.watch !== "undefined" ? typeof optionsForStatic.watch === "boolean" ? optionsForStatic.watch ? def.watch : false : getWatchOptions(optionsForStatic.watch) : def.watch
        };
      }
    ...
    if (typeof options.server === "function" || typeof options.server === "string") {
      options.server = {
        type: options.server,
        options: {}
      };
    } else {
      const serverOptions = /** @type {ServerConfiguration<A, S>} */
      options.server || {};
      options.server = {
    ...
    if (options.server.type === "https" || options.server.type === "http2") {
      ...
      const httpsProperties = /** @type {(keyof ServerOptions)[]} */
      ["ca", "cert", "crl", "key", "pfx"];
      for (const property_ of httpsProperties) {
        ...
        const readFile = item => {
          ...
          if (item) {
            let stats = null;
            try {
              stats = _gracefulFs.default.lstatSync(_gracefulFs.default.realpathSync(item)).isFile();
            } catch {
              // Ignore error
            }

            // It is a file
            return stats ? _gracefulFs.default.readFileSync(item) : item;
          }
        };

        /** @type {EXPECTED_ANY} */
        serverOptions[property] = Array.isArray(value) ? value.map(item => readFile(/** @type {string | Buffer | undefined} */
        item)) : readFile(/** @type {string | Buffer | undefined} */
        value);
      }
      let fakeCert;
      if (!serverOptions.key || !serverOptions.cert) {
        const certificateDir = Server.findCacheDir();
        const certificatePath = _nodePath.default.join(certificateDir, "server.pem");
        let certificateExists;
        try {
          const certificate = await _gracefulFs.default.promises.stat(certificatePath);
        ...
        if (certificateExists) {
          ...
          const certificateStat = await _gracefulFs.default.promises.stat(certificatePath);
          const now = Date.now();

          // cert is more than 30 days old, kill it with fire
          if ((now - Number(certificateStat.ctime)) / certificateTtl > 30) {
            this.logger.info("SSL certificate is more than 30 days old. Removing...");
            await _gracefulFs.default.promises.rm(certificatePath, {
              recursive: true
            });
            certificateExists = false;
          }
        ...
        if (!certificateExists) {
          ...
          await _gracefulFs.default.promises.mkdir(certificateDir, {
            recursive: true
          });
          await _gracefulFs.default.promises.writeFile(certificatePath, pems.private + pems.cert, {
            encoding: "utf8"
          });
        }
        fakeCert = await _gracefulFs.default.promises.readFile(certificatePath);
    ...
    if (typeof options.ipc === "boolean") {
      ...
      const pipePrefix = isWindows ? "\\\\.\\pipe\\" : _nodeOs.default.tmpdir();
      const pipeName = `${pluginName}.sock`;
      options.ipc = _nodePath.default.join(pipePrefix, pipeName);
    }
   ...
   */
  #getColors() {
    const compilerOptions = /** @type {MultiCompiler} */
    ...
    return {
      isColorSupported: compilerOptions.cli.isColorSupported,
      colors
    };
  }
    ...
    switch (typeof clientTransport) {
      case "string":
        ...
        if (clientTransport === "ws") {
          clientImplementation = cjsRequire.resolve("../client/clients/WebSocketClient.js");
        } else {
          try {
  ...
  async getServerTransport() {
    ...
    this.options.webSocketServer.type)) {
      case "string":
        ...
        if (/** @type {WebSocketServerConfiguration} */this.options.webSocketServer.type === "ws") {
          implementation = (await Promise.resolve().then(() => require("./servers/WebsocketServer.js"))).default;
        } else {
          try {
            const mod = cjsRequire(/** @type {string} */
            /** @type {WebSocketServerConfiguration} */
            this.options.webSocketServer.type);
    ...
    if (!implementationFound) {
      throw new Error("webSocketServer (webSocketServer.type) must be a string denoting a default implementation (e.g. 'ws'), a resolvable module specifier or absolute file path " + "which exports a class extending BaseServer (webpack-dev-server/lib/servers/BaseServer.js), " + "or the class itself which extends BaseServer");
    }
  ...
  async initialize() {
    ...
    if (this.options.webSocketServer) {
      ...
      for (const compiler of compilers) {
        ...
        if (compiler.options.output.module && compiler.platform.web === null && compiler.platform.node === null) {
          let emitter;
          try {
          ...
          if (emitter) {
            new webpack.NormalModuleReplacementPlugin(/emitter(\.js)?$/, result => {
              if (/webpack[/\\]hot|webpack-dev-server[/\\]client/.test(result.context)) {
        ...
        if (this.options.hot) {
          const HMRPluginExists = compiler.options.plugins.find(plugin => plugin && plugin.constructor === webpack.HotModuleReplacementPlugin);
          if (HMRPluginExists) {
    ...
    const webSocketProxies = /** @type {RequestHandler[]} */
    this.webSocketProxies;
    const hmrPath = this.options.webSocketServer && /** @type {WebSocketServerConfiguration} */
    this.options.webSocketServer.options && /** @type {NonNullable<WebSocketServerConfiguration["options"]>} */
    (/** @type {WebSocketServerConfiguration} */
    this.options.webSocketServer.options).path;
    for (const webSocketProxy of webSocketProxies) {
      const proxyUpgrade = /** @type {RequestHandler & { upgrade: NonNullable<RequestHandler["upgrade"]> }} */
      ...
      this.server.on("upgrade", (req, socket, head) => {
        if (hmrPath && typeof req.url === "string") {
          ...
          const pathname = queryIndex !== -1 ? req.url.slice(0, queryIndex) : req.url;
          if (pathname === hmrPath) {
            return;
          }
        }
  ...
  async setupApp() {
    ...
    typeof this.options.app === "function" ? await this.options.app() : (await getExpress())();
  }
  ...
  getStats(statsObj) {
    ...
    const compilerOptions = this.getCompilerOptions();
    if (compilerOptions.stats && /** @type {StatsOptions} */compilerOptions.stats.warningsFilter) {
      stats.warningsFilter = /** @type {StatsOptions} */
  ...
  async setupMiddlewares() {
    ...
    middlewares.push({
      ...
      middleware: (req, res, next) => {
        const headers = /** @type {{ [key: string]: string | undefined }} */
        req.headers;
    ...
    middlewares.push({
      ...
      middleware: (req, res, next) => {
        ...
        if (headers["sec-fetch-mode"] === "no-cors" && headers["sec-fetch-site"] === "cross-site") {
          res.statusCode = 403;
        ...
        if (this.options.allowedHosts !== "all" && !this.isUserCORSWildcardEnabled()) {
          res.setHeader("Cross-Origin-Resource-Policy", "same-origin");
        }
        next();
      }
    });
    const isHTTP2 = /** @type {ServerConfiguration<A, S>} */this.options.server.type === "http2";
    if (isHTTP2) {
      ...
      middlewares.push({
        ...
        middleware: /** @type {NextHandleFunction} */
        (_req, res, next) => {
    ...
    middlewares.push({
      ...
      middleware: async (req, res, next) => {
        ...
        if (typeof fileName === "string") {
          const {
            default: launchEditor
          } = await import("launch-editor");
    ...
    middlewares.push({
      ...
      middleware: (req, res, next) => {
        ...
        this.middleware.waitUntilValid(stats => {
           ...
           */
          const statsForPrint = typeof (/** @type {MultiStats} */stats.stats) !== "undefined" ? (/** @type {NonNullable<StatsCompilation["children"]>} */
          /** @type {MultiStats} */stats.toJson().children) : [/** @type {Stats} */stats.toJson()];
          res.write("<h1>Assets Report:</h1>");
          for (const [index, item] of statsForPrint.entries()) {
            res.write("<div>");
            const name = typeof item.name !== "undefined" ? item.name : /** @type {MultiStats} */stats.stats ? `unnamed[${index}]` : "unnamed";
            res.write(`<h2>Compilation: ${name}</h2>`);
            res.write("<ul>");
            const publicPath = item.publicPath === "auto" ? "" : item.publicPath;
            const assets = /** @type {NonNullable<StatsCompilation["assets"]>} */
            item.assets;
    ...
    if (this.options.proxy) {
      ...
      const getProxyMiddleware = proxyConfig => {
        const context = proxyConfig.context || proxyConfig.path || proxyConfig.pathFilter;
    ...
    const staticOptions = /** @type {NormalizedStatic[]} */
    this.options.static;
      ...
      for (const staticOption of staticOptions) {
        for (const publicPath of staticOption.publicPath) {
          middlewares.push({
            name: "express-static",
            path: publicPath,
            middleware: (await getExpress()).static(staticOption.directory, staticOption.staticOptions)
          });
        }
      }
    }
    ...
    const lazyInitDevMiddleware = async () => {
      if (!this.middleware) {
        ...
        // @ts-expect-error
        this.compiler, this.options.devMiddleware, this.isPlugin);
      }
    ...
    for (const middleware of middlewares) {
      if (typeof middleware === "function") {
        /** @type {A} */
        this.app.use(/** @type {NextHandleFunction | HandleFunction} */
        middleware);
      } else if (typeof middleware.path !== "undefined") {
        /** @type {A} */
        this.app.use(middleware.path, /** @type {SimpleHandleFunction | NextHandleFunction} */
  ...
  async createServer() {
    ...
    if (typeof type === "function") {
      /** @type {S | undefined} */
      this.server = await type(/** @type {ServerOptions} */
      options, /** @type {A} */
      this.app);
    } else {
      ...
      /** @type {S | undefined} */
      this.server = type === "http2" ? serverType.createSecureServer({
        ...options,
        allowHTTP1: true
      }, this.app) : serverType.createServer(options, this.app);
    }
    this.isTlsServer = typeof (/** @type {import("tls").Server} */this.server.setSecureContext) !== "undefined";
  ...
  async createWebSocketServer() {
    ...
    (client, request) => {
      ...
      const headers = typeof request !== "undefined" ? (/** @type {{ [key: string]: string | undefined }} */
  ...
  async openBrowser(defaultOpenTarget) {
    ...
    this.options.open.map(item => {
      ...
      return open(openTarget, item.options).catch(() => {
        this.logger.warn(`Unable to open "${openTarget}" page${item.options.app ? ` in "${ /** @type {import("open").App} */
        item.options.app.name}" app${ /** @type {import("open").App} */
        item.options.app.arguments ? ` with "${ /** @type {import("open").App} */
        item.options.app.arguments.join(" ")}" arguments` : ""}` : ""}. If you are running in a headless environment, please do not use the "open" option or related flags like "--open", "--open-target", and "--open-app-name".`);
      });
    }));
  }
  ...
  async runBonjour() {
    ...
    this.bonjour.publish({
      name: `Webpack Dev Server ${_nodeOs.default.hostname()}:${this.options.port}`,
  ...
  async logStatus() {
    ...
    if (this.options.ipc) {
       ...
       */
      const prettyPrintURL = newHostname => _nodeUrl.default.format({
        protocol,
      ...
      if (this.options.host) {
        if (this.options.host === "localhost") {
          ...
          let isIP;
          try {
            isIP = _ipaddr.default.parse(this.options.host);
          } catch {
            // Ignore
          }
          if (!isIP) {
      ...
      const parsedIP = _ipaddr.default.parse(address);
      ...
      if (/** @type {NormalizedOpen[]} */this.options.open.length > 0) {
        const openTarget = prettyPrintURL(!this.options.host || this.options.host === "0.0.0.0" || this.options.host === "::" ? "localhost" : this.options.host);
  ...
  isHostAllowed(value) {
    ...
    if (Array.isArray(allowedHosts) && allowedHosts.length > 0) {
      for (const allowedHost of allowedHosts) {
        ...
        value === allowedHost.slice(1) || /** @type {string} */
    ...
    if (this.options.client && typeof (/** @type {ClientConfiguration} */
    this.options.client.webSocketURL) !== "undefined") {
      return /** @type {WebSocketURL} */(/** @type {ClientConfiguration} */this.options.client.webSocketURL).hostname === value;
    }
    return false;
  }
  ...
  #parseHostnameFromHeader = function (header) {
    ...
    try {
      ...
      const parseUrl = new URL(/^(.+:)?\/\//.test(header) ? header : `//${header}`, "http://localhost/");
  ...
  isUserCORSWildcardEnabled() {
    ...
    if (Array.isArray(headers)) {
      return headers.some(header => header.key.toLowerCase() === "access-control-allow-origin" && isWildcard(header.value));
    }
    return Object.entries(headers).some(([key, value]) => key.toLowerCase() === "access-control-allow-origin" && isWildcard(value));
  }
  ...
  isValidHost(headers, headerToCheck, validateHost = true) {
    ...
    const isValidHostname = validateHost ? _ipaddr.default.IPv4.isValid(hostname) || _ipaddr.default.IPv6.isValid(hostname) || hostname === "localhost" || hostname.endsWith(".localhost") || hostname === this.options.host : false;
  ...
  isSameOrigin(headers) {
    ...
    const loopbackPermitted = this.options.allowedHosts === "auto" || [...loopbacks].some(alias => this.isHostAllowed(alias));
  ...
  async watchFiles(watchPath, watchOptions = {}) {
    ...
    const isWin = _nodePath.default.sep === "\\";
    const toPosix = (/** @type {string} */filePath) => isWin ? filePath.split(_nodePath.default.sep).join("/") : filePath;
    const toNative = (/** @type {string} */filePath) => isWin ? filePath.split("/").join(_nodePath.default.sep) : filePath;
    const cwd = watchOptions.cwd ? toPosix(watchOptions.cwd) : undefined;
    const expand = (/** @type {string} */item) => {
      const posix = toPosix(item);
      return isDynamicPattern(posix) ? globSync(posix, {
        cwd,
        absolute: true
      }).map(toNative) : item;
    };
    const resolveGlobs = (/** @type {string | string[]} */input) => (Array.isArray(input) ? input : [input]).flatMap(item => typeof item === "string" ? expand(item) : item);
    ...
    if (typeof watchOptions.ignored === "string") {
      ...
      watchOptions.ignored = watchOptions.ignored.flatMap(item => typeof item === "string" ? expand(item) : item);
    }
  ...
  invalidate(callback = () => {}) {
    ...
    if (this.isPlugin) {
      const compilers = /** @type {MultiCompiler} */this.compiler.compilers || (/** @type {Compiler[]} */[this.compiler]);
      ...
      new Promise((resolve, reject) => {
        ...
        error => {
          if (error.code === "ECONNREFUSED") {
            ...
            _gracefulFs.default.unlinkSync(/** @type {string} */this.options.ipc);
            resolve();
            return;
  ...
  async listen() {
    ...
    if (this.options.ipc) {
      ...
      await _gracefulFs.default.promises.chmod(/** @type {string} */this.options.ipc, READ_WRITE);
    }
    if (this.options.webSocketServer) {
  ...
  apply(compiler) {
    ...
    hooks.done.tap(pluginName, () => {
      ...
      listening = true;
      ensureSetup().then(() => this.listen()).catch(error => {
    ...
    const childCompilers = /** @type {MultiCompiler} */compiler.compilers || [compiler];
    for (const childCompiler of childCompilers) {
      childCompiler.hooks.shutdown.tapPromise(pluginName, onShutdown);
    }
  }
}
var _default = exports.default = Server;module.exports = exports.default;
